...
CHomingToFoodBeaconBehavior::CHomingToFoodBeaconBehavior(UInt8 BeaconRobotId, Real MAX_BEACON_SIGNAL_RANGE)
{
    m_iBeaconRobotId     = BeaconRobotId;
...
bool CHomingToFoodBeaconBehavior::TakeControl()
{
    ...
    for(size_t i = 0; i <  m_sSensoryData.m_RABSensorData.size(); ++i)
    {
        ...
        if(m_sSensoryData.m_RABSensorData[i]->RobotId == m_iBeaconRobotId && m_sSensoryData.m_RABSensorData[i]->Range < m_fBeaconSignalRange)
        {
            controltaken = true;
            ...
            if(m_sSensoryData.m_RABSensorData[i]->Range < closestBeaconRange)
            {
                ...
                closestBeaconBearing = m_sSensoryData.m_RABSensorData[i]->Bearing;
            }
        }
    }
