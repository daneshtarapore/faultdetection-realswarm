...
/**
  Modified from http://stackoverflow.com/a/24235744
*/
...
double   CBayesianInferenceFeatureVector::FEATURE_RANGE             = 30.0;
...
unsigned CBayesianInferenceFeatureVector::GetIdFromRABPacket(CCI_EPuckPseudoRangeAndBearingSensor::TPackets& rab_packet, size_t rab_packet_index)
{
    return rab_packet[rab_packet_index]->RobotId;
...
unsigned CBayesianInferenceFeatureVector::GetIdFromRABPacket(CCI_EPuckPseudoRangeAndBearingSensor::TPackets& rab_packet, size_t rab_packet_index, unsigned& range)
{
    //std::cout << "In function GetIdFromRABPacket " << std::endl;
    //std::cout << "RAB Packet Size " << rab_packet.size() << " Index " << rab_packet_index << " Robot Id " << rab_packet[rab_packet_index]->RobotId << " Range " << (unsigned)((rab_packet[rab_packet_index])->Range)<< std::endl;
...
float CBayesianInferenceFeatureVector::GetAnguAccelerFromRABPacket(CCI_EPuckPseudoRangeAndBearingSensor::TPackets& rab_packet, size_t rab_packet_index, unsigned observerd_robot_id)
{
    assert(rab_packet[rab_packet_index]->RobotId == observerd_robot_id);
    return (((Real)(rab_packet[rab_packet_index]->Data[0]) / 255.0) * 2.0f) - 1.0f; // return [-1, +1]
    ...
    */
    ...
    if (m_fEstimated_Dist_LongTimeWindow != -1.0f)
    {
        ...
        if(list_Dist_LongRangeTimeWindow.size() >= 100u)
        {
            ...
            list_Dist_LongRangeTimeWindow.front() = std::move(list_Dist_LongRangeTimeWindow.back());
            list_Dist_LongRangeTimeWindow.pop_back();
        }
    ...
    if(m_fEstimated_Dist_MediumTimeWindow != -1.0f)
    {
        list_Dist_MediumRangeTimeWindow.push_back(m_fEstimated_Dist_MediumTimeWindow);
        if(list_Dist_MediumRangeTimeWindow.size() >= 50u)
        {
            //list_Dist_MediumRangeTimeWindow.pop_front();
            list_Dist_MediumRangeTimeWindow.front() = std::move(list_Dist_MediumRangeTimeWindow.back());
            list_Dist_MediumRangeTimeWindow.pop_back();
        }
...
Real CBayesianInferenceFeatureVector::BayesInference_ObservedRobots_FeatureVector::CountNeighbors(Real lb_sensor_range, Real hb_sensor_range, Real& dist_nearest_nbr, Real& CoM_nbrs, Real *Range_ObserverToObserved)
{
    ...
    for(size_t i = 0; i <  owner.m_sSensoryData.m_RABSensorData.size(); ++i)
    {
        ...
        if(observedRobotId_1 == observedRobotId_2)
            continue;

        Real observedRobotId_2_Range       = owner.m_sSensoryData.m_RABSensorData[i]->Range;
        CRadians observedRobotId_2_Bearing = owner.m_sSensoryData.m_RABSensorData[i]->Bearing;
